...
import { Cmyk, ColorPickerModule, ColorPickerService } from 'ngx-color-picker';
import { EstadosReservasService } from '../Services/estados-reservas.service';


@Component({
  selector: 'app-form-estado-reserva',
  templateUrl: './form-estado-reserva.page.html',
  styleUrls: ['./form-estado-reserva.page.scss'],
})
export class FormEstadoReservaPage implements OnInit {
  ...
  constructor(
    ...
    private estadoService:EstadosReservasService,
  ...
  guardar(){
    this.estadoService.add(this.estado).then(() => {
  ...
  cerrar(){
    this.modalCtrl.dismiss('form-estado-reserva')
  }
